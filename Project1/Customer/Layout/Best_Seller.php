...
<head>
    ...
    $sqlCountRecord = "SELECT COUNT(*) AS count_record FROM books INNER JOIN order_details ON 
                        order_details.book_id = books.id INNER JOIN orders ON order_details.order_id = orders.id
                        WHERE (name LIKE '%$search%') AND (date_buy between '$last7days' AND '$today')";
